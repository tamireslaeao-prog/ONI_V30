...
### Problema: "JSON não está definido" (ReferenceError)
**Causa**: Photoshop usa JavaScript ES3 (1999) sem suporte nativo a JSON
**Solução**: ✅ **JÁ CORRIGIDO NA V4**
- O script agora inclui polyfill completo de JSON.stringify e JSON.parse
- Funciona em TODAS as versões do Photoshop (CS6 até 2026)

### Problema: "Arquivo ONI_STYLE_DATA.json vazio (0 bytes)"
**Causa**: Script travou antes de escrever dados
**Solução**:
1. Verifique se há EXATAMENTE UMA layer com efeitos visível
2. Abra o PSD fonte primeiro, DEPOIS execute o script
3. Não clique em nada durante a extração (aguarde 5-10 segundos)
4. Verifique `ONI_DEBUG_EXTRACTOR.txt` no Desktop para ver erros
...
### Problema: "Script não encontra layers com efeitos"
**Solução**:
1. Certifique-se que a layer tem Layer Styles (não Smart Filters)
2. A layer deve estar VISÍVEL (ícone de olho ativado)
3. Não pode ser layer travada (locked)
4. Grupos (LayerSets) são escaneados recursivamente
