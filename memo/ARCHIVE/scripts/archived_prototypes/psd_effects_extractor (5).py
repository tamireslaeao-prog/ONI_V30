...
def main():
    ...
    # Criar pasta para os JSONs
    output_dir = Path('psd_json_output')
    ...
    for psd_file in psd_files:
        try:
            # Extrair dados do PSD
            psd_data = extract_psd_complete(str(psd_file))
            
            # Gerar nome do arquivo JSON baseado no nome do PSD
            json_filename = psd_file.stem + '.json'
            json_path = output_dir / json_filename
            
            # Salvar JSON individual
            with open(json_path, 'w', encoding='utf-8') as f:
                json.dump(psd_data, f, indent=2, ensure_ascii=False)
            
            file_size = os.path.getsize(json_path) / 1024  # KB
            total_size += file_size
            
            print(f"✅ Salvo: {json_filename} ({file_size:.1f} KB)")
            processed_count += 1
    ...
    print(f"{'='*70}")
    print(f"📊 Arquivos processados: {processed_count}/{len(psd_files)}")
    print(f"📁 Pasta de saída: {output_dir}/")
    print(f"📦 Tamanho total: {total_size/1024:.2f} MB")
    print(f"\n🎯 Dados extraídos em cada JSON:")
