...
function Get-CorelCOMObject {
    ...
    for ($i = 1; $i -le $RetryCount; $i++) {
        try {
            Write-NexusLog "Tentando conectar ao CorelDRAW (tentativa $i/$RetryCount)..." -Level Debug
            
            $corel = [System.Runtime.InteropServices.Marshal]::GetActiveObject("CorelDRAW.Application")
            
            if ($null -ne $corel) {
                Write-NexusLog "Objeto COM obtido com sucesso" -Level Debug
                return $corel
            }
        }
        catch {
            $errMsg = $_.Exception.Message
            Write-NexusLog "Falha na tentativa ${i}: $errMsg" -Level Debug
...
function New-CorelCOMObject {
    ...
    try {
        ...
        throw "Falha ao criar instancia COM: $_"
    }
}
