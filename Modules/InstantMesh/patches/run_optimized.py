...
# Use offloading for 4GB VRAM support
try:
    pipeline.enable_model_cpu_offload()
    print("Enabled model CPU offloading for low VRAM.")
    # Force VAE to float32 to avoid black images on some setups (optional but safe)
    # pipeline.vae.to(dtype=torch.float32) 
except Exception as e:
    print(f"Warning: Could not enable CPU offload: {e}")
    pipeline = pipeline.to(device)

# Reconstruction model loading moved to Stage 2 to save VRAM
...
# load reconstruction model (Delayed Load)
